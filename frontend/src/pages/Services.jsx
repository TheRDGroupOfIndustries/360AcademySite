...
const Card = memo(function Card({ title, price, img, priority }) {
  return (
    <motion.div
      ...
      initial={false}
      className="bg-[#fffaf3] rounded-2xl shadow-lg overflow-hidden border-l-8 border-yellow-500 will-change-transform flex flex-col md:flex-row"
    >
      {/* Image */}
      ...
      <div className="p-6 md:w-2/3 flex flex-col justify-center">
        ...
        <button className="mt-4 w-fit px-6 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-full transition">
          Book Now
        </button>
      </div>
    </motion.div>
      ...
      <div className="max-w-6xl mx-auto px-6 flex flex-col gap-10">
        {services.map((service, index) => (
          <Card
            key={index}
            {...service}
            priority={index < 2} // only top 2 eager
          />
        ))}
      </div>
    </div>
